...
def build_runtime_diagnostics_report(records: list[PriceRecord], runtime: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- run_time_budget_exceeded: {runtime.get('run_time_budget_exceeded', False)}",
        f"- runtime_warning_level: {runtime.get('runtime_warning_level', 'runtime_info')}",
        f"- runtime_warning_reason: {runtime.get('runtime_warning_reason', 'within_runtime_policy')}",
...
def _minute_bars_completeness_lines(records: list[PriceRecord], runtime: dict[str, Any]) -> list[str]:
    ...
    operation_candidates = [
        record
        for record in records
        if record.source_universe == "tech_operation_candidates" or record.universe_type == "operation_candidate"
    ]
    ...
    lines = [
        ...
        f"- operation_candidate_minute_missing_count: {sum(1 for record in operation_candidates if not record.minute_bars_available)}",
